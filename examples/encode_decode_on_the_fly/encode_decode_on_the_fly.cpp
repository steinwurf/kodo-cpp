...
int main(void)
{
    ...
    while (!decoder.is_complete())
    {
        ...
        std::cout << "Payload generated by encoder, rank = " << encoder.rank()
                  << ", bytes used = " << bytes_used << std::endln;

        if (rand() % 2)
        {
            std::cout << "packet dropped" << std::endln;
            continue;
        }
            ...
            decoder.is_partial_complete())
        {
            for (uint32_t i = 0; i < decoder.symbols(); ++i)
            {
                ...
                if (!decoded[i])
                {
                    ...
                    std::cout << "Symbol " << i << " was decoded" << std:endln;
                    decoded[i] = true;
                    ...
                    {
                        std::cout << "Symbol " << i << " decoded correctly." << std::endln;
                    }
                    else
                    {
                        std::cout << "SYMBOL " << i << "DECODING FAILED." << std::endln;
                    }
                }
            }
        }
